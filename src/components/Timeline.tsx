...
export function Timeline({ items }: { items: TimelineItemT[] }) {
  return (
    <ol className="relative">
      <div
        className="absolute left-4 sm:left-1/2 top-0 bottom-0 w-px bg-border -translate-x-px"
        aria-hidden
      />
      {items.map((it, i) => {
        const left = i % 2 === 0;
        ...
        return (
          <li key={i} className="relative pl-12 sm:pl-0 sm:grid sm:grid-cols-2 sm:gap-8 mb-10">
            <div
              className={`hidden sm:block ${left ? "sm:pr-10 sm:text-right" : "sm:col-start-2 sm:pl-10"}`}
            >
              <motion.div
